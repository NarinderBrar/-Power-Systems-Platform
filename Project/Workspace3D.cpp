...
// Workspace3D
Workspace3D::Workspace3D() : WorkspaceBase( nullptr )
{
}
Workspace3D::Workspace3D( wxWindow* parent, wxString name, wxStatusBar* statusBar, wxGLContext* sharedGLContext ): WorkspaceBase( parent )
{
    m_timer->Start();
    m_name = name;
...
void Workspace3D::OnPaint( wxPaintEvent& event )
{
    ...
    m_projMatrix = glm::perspective( glm::radians( 65.0f ), (float)width / (float)height, 1.0f, 5000.0f );
...
void Workspace3D::OnLeftClickDown( wxMouseEvent& event )
{
...
void Workspace3D::OnLeftDoubleClick( wxMouseEvent& event )
{
...
void Workspace3D::OnRightClickDown( wxMouseEvent& event )
{
    bool redraw = false;
            ...
            if( element->IsSelected() )
            {
                ...
                if( element->Contains( m_camera->ScreenToWorld( event.GetPosition() ) ) )
                {
                    ...
                    if( element->GetContextMenu( menu ) )
                    {
                        ...
                        menu.Bind( wxEVT_COMMAND_MENU_SELECTED, &Workspace3D::OnPopupClick, this );
                        PopupMenu( &menu );
                        redraw = true;
...
void Workspace3D::OnLeftClickUp( wxMouseEvent& event )
{
...
void Workspace3D::OnMouseMotion( wxMouseEvent& event )
{
    bool redraw = false;
    switch( m_mode )
    {
...
void Workspace3D::OnMiddleDown( wxMouseEvent& event )
{
    // Set to drag mode.
    switch( m_mode )
    {
...
void Workspace3D::OnMiddleUp( wxMouseEvent& event )
{
    switch( m_mode )
    {
        case WorkspaceMode::MODE_DRAG_INSERT:
...
void Workspace3D::OnScroll( wxMouseEvent& event )
{
...
void Workspace3D::OnKeyDown( wxKeyEvent& event )
{
        ...
        switch( key )
        {
            ...
            case 'B':  // Insert a bus.
            {
                if( !insertingElement )
                {
                    Bus* newBus = new Bus( m_camera->ScreenToWorld( event.GetPosition() ),
                                           wxString::Format( _( "Bus %d" ), GetElementNumber( ID_BUS_3D ) ) );
                    IncrementElementNumber( ID_BUS_3D );
            ...
            case 'L':
            {
                if( !insertingElement )
                {
                    ...
                    {  // Insert a load.
                        Load* newLoad = new Load( wxString::Format( _( "Load %d" ), GetElementNumber( ID_LOAD_3D ) ) );
                        IncrementElementNumber( ID_LOAD_3D);
                    ...
                    {  // Insert a power line.
                        Line* newLine = new Line( wxString::Format( _( "Line %d" ), GetElementNumber( ID_LINE_3D ) ) );
                        IncrementElementNumber( ID_LINE_3D );
            ...
            case 'T':  // Insert a transformer.
            {
                if( !insertingElement )
                {
                    Transformer* newTransformer =
                        new Transformer( wxString::Format( _( "Transformer %d" ), GetElementNumber( ID_TRANSFORMER_3D ) ) );
                    IncrementElementNumber( ID_TRANSFORMER_3D );
            ...
            case 'G':  // Insert a generator.
            {
                if( !insertingElement )
                {
                    SyncGenerator* newGenerator =
                        new SyncGenerator( wxString::Format( _( "Generator %d" ), GetElementNumber( ID_SYNCGENERATOR_3D ) ) );
                    IncrementElementNumber( ID_SYNCGENERATOR_3D );
                ...
                if( !insertingElement )
                {
                    ...
                    {  // Insert an inductor.
                        Inductor* newInductor =
                            new Inductor( wxString::Format( _( "Inductor %d" ), GetElementNumber( ID_INDUCTOR_3D ) ) );
                        IncrementElementNumber( ID_INDUCTOR_3D );
                    ...
                    else  // Insert an induction motor.
                    {
                        IndMotor* newIndMotor =
                            new IndMotor( wxString::Format( _( "Induction motor %d" ), GetElementNumber( ID_INDMOTOR_3D ) ) );
                        IncrementElementNumber( ID_INDMOTOR_3D );
            ...
            case 'K':  // Insert a synchronous condenser.
            {
                if( !insertingElement )
                {
                    SyncMotor* newSyncCondenser =
                        new SyncMotor( wxString::Format( _( "Synchronous condenser %d" ), GetElementNumber( ID_SYNCMOTOR_3D ) ) );
                    IncrementElementNumber( ID_SYNCMOTOR_3D );
                ...
                if( !insertingElement )
                {
                    ...
                    {  // Insert a capacitor.
                        Capacitor* newCapacitor =
                            new Capacitor( wxString::Format( _( "Capacitor %d" ), GetElementNumber( ID_CAPACITOR_3D ) ) );
                        IncrementElementNumber( ID_CAPACITOR_3D );
            ...
            case 'H':
            {
                if( !insertingElement )
                {
                    ...
                    {  // Insert an harmonic current source.
                        HarmCurrent* newHarmCurrent = new HarmCurrent(
                            wxString::Format( _( "Harmonic Current %d" ), GetElementNumber( ID_HARMCURRENT_3D ) ) );
                        IncrementElementNumber( ID_HARMCURRENT_3D );
...
void Workspace3D::UpdateStatusBar()
{
    switch( m_mode )
    {
        case WorkspaceMode::MODE_DRAG:
...
void Workspace3D::UpdateHeatMap()
{
    if( m_hmPlane && m_showHM )
    {
        m_hmPlane->Clear();
...
void Workspace3D::OnPopupClick( wxCommandEvent& event )
{
    bool redrawHM = false;
...
void Workspace3D::RotateSelectedElements( bool clockwise )
{
    for( auto it = m_elementList.begin(); it != m_elementList.end(); ++it )
    {
        Element* element = *it;
...
void Workspace3D::DeleteSelectedElements()
{
...
bool Workspace3D::GetElementsCorners( wxPoint2DDouble& leftUpCorner, wxPoint2DDouble& rightDownCorner, std::vector<Element*> elementList )
{
...
void Workspace3D::ValidateBusesVoltages( Element* initialBus )
{
...
void Workspace3D::ValidateElementsVoltages()
{
    for( auto it = m_elementList.begin(); it != m_elementList.end(); it++ )
    {
...
bool Workspace3D::UpdateTextElements()
{
...
void Workspace3D::CopySelection()
{
    UpdateElementsID();
...
void Workspace3D::EnableHeatMap( const bool& enable )
{
    m_showHM = enable;
    if( m_showHM )
    {
...
void Workspace3D::UpdateElementsID()
{
    int id = 0;
    for( auto it = m_elementList.begin(), itEnd = m_elementList.end(); it != itEnd; ++it )
    {
...
void Workspace3D::OnTimer( wxTimerEvent& event )
{
    if( m_tipWindow )
    {
        m_tipWindow->Close();
...
void Workspace3D::SetTextList( std::vector<Text*> textList )
{
    m_textList.clear();
...
void Workspace3D::SetElementList( std::vector<Element*> elementList )
{
    m_elementList.clear();
...
void Workspace3D::OnIdle( wxIdleEvent& event )
{
...
std::vector<Element*> Workspace3D::GetAllElements() const
{
...
std::vector<Element*> Workspace3D::GetElementList() const
{
...
void Workspace3D::OnMiddleDoubleClick( wxMouseEvent& event )
{
    Fit();
    event.Skip();
}

bool Workspace3D::RunStaticStudies()
{
...
bool Workspace3D::RunHarmonicDistortion()
{
    auto simProp = m_properties->GetSimulationPropertiesData();
    double basePower = simProp.basePower;
    if( simProp.basePowerUnit == ElectricalUnit::UNIT_MVA )
...
bool Workspace3D::RunFrequencyResponse()
{
    // Get bus list
    std::vector<Bus*> busList;
    for( auto it = m_elementList.begin(), itEnd = m_elementList.end(); it != itEnd; ++it )
...
void Workspace3D::OnResize( wxSizeEvent& event )
{
...
void Workspace3D::OnHeatMapTime( wxTimerEvent& event )
{
    if( m_showHMTimer )
    {
        UpdateHeatMap();
